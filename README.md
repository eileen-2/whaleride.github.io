...
# 제주도를 바이크로 여행하는 모험가들을 위한 커뮤니티 서비스
...
## 9. 메인 기능
- **오토바이/스쿠터 렌탈 서비스**: 여러 종류의 오토바이나 스쿠터를 미리 예약하고 렌탈할 수 있도록 도와주는 기능으로 렌탈 가능한 차량의 사진/특징/가격 등을 보여주고 렌탈샵으로 안내

- **맞춤형 여행 일정 및 경로 추천**: 사용자 지정 필수 여행지, 제주도의 날씨 등을 고려하여 맞춤형 여행 일정 및 경로를 추천

- **라이더 커뮤니티**: 사용자들이 자신의 여행 경험을 공유하고 팁을 주고 받는 커뮤니티 플랫폼 제공

```mermaid
    graph TD
        A[추천 시작] --> B[사용자 선호도 및 상황 입력]
        B --> C[경로 생성]
        C --> D{경로에 만족하시나요?}
        D --> |네| E[여행 시작]
        D --> |아니오| B
...
```mermaid
    sequenceDiagram
        사용자A->>커뮤니티: 새로운 경험 공유
        커뮤니티->>사용자B: 사용자A의 경험 알림
        사용자B->>커뮤니티: 사용자A의 경험에 대한 반응
        커뮤니티->>사용자A: 사용자B의 반응 알림
```
...
## 10. 개발하며 느낀점
- 강의를 들어도 HTML/JS/CSS 각각 고유의 영역에 대한 구분이 명확하지 않았는데 프로젝트를 진행하면서 각각의 범위를 명확하게 알 수 있었다.  
